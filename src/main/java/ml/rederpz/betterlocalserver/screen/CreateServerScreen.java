...
public final class CreateServerScreen extends Screen {
    ...
    private SliderWidget viewDistanceSlider;
    ...
    @Override
    protected void init() {
        this.addButton(this.motdTextField = new TextFieldWidget(this.textRenderer, this.width / 2 - 155, 75, 300, 20, new TranslatableText("internalServer.motd")));
        this.motdTextField.setText(this.client.getServer().getServerMotd());
        this.motdTextField.setEditable(false);

        this.addButton(this.gameModeButton = new ButtonWidget(this.width / 2 - 155, 100, 150, 20, LiteralText.EMPTY, (buttonWidget) -> {
        ...
        this.addButton(this.cheatsCheckbox = new CheckboxWidget(this.width / 2 + 5, 100, 100, 20, new TranslatableText("internalServer.allowCheats"), this.client.getServer().getPlayerManager().areCheatsAllowed()));
        this.addButton(this.flightCheckbox = new CheckboxWidget(this.width / 2 + 5, 125, 100, 20, new TranslatableText("internalServer.allowFlight"), this.client.getServer().isFlightEnabled()));
        this.addButton(this.offlineCheckbox = new CheckboxWidget(this.width / 2 + 5, 150, 100, 20, new TranslatableText("internalServer.offlineMode"), !this.client.getServer().isOnlineMode()));
        this.addButton(this.pvpCheckbox = new CheckboxWidget(this.width / 2 + 5, 175, 100, 20, new TranslatableText("internalServer.pvp"), this.client.getServer().isPvpEnabled()));
        ...
        this.addButton(this.viewDistanceSlider = new SliderWidget(this.width / 2 - 155, 125, 150, 20, new TranslatableText("internalServer.viewDistance", viewDistance), viewDistancePercent) {
            @Override
        ...
        this.addButton(new ButtonWidget(this.width / 2 - 155, this.height - 28, 150, 20, new TranslatableText("internalServer.start"), (buttonWidget) -> {
    ...
    public void render(final MatrixStack matrices, final int mouseX, final int mouseY, final float delta) {
        ...
        DrawableHelper.drawCenteredText(matrices, this.textRenderer, this.title, this.width / 2, 50, 0xFFFFFFFF);
        DrawableHelper.drawCenteredText(matrices, this.textRenderer, this.title, this.width / 2, 50, 0xFFFFFFFF);
